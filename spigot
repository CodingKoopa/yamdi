...
if [ "$FORCE_SPIGOT_REBUILD" = true ] || [ ! -f "$SPIGOT_REVISION_JAR" ]; then
  ...
  # Configuration files are not put in a seperate directory, but are instead scattered around the working directory. So
  ...
  for FILE in "${CONFIGURATION_FILES[@]}"; do
    touch "$SPIGOT_CONFIG_DIRECTORY/$FILE"
    ln -sf "$SPIGOT_CONFIG_DIRECTORY/$FILE" "$SPIGOT_DIRECTORY/$FILE"
  done
